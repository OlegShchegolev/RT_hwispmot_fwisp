...
cl_int3		*rt_cl(t_cl *cl, t_scene scene)
{
    ...
    set_ker_arg(cl);
	// printf("AAOx %f\n", scene.sources->intensities.x);
	// printf("AAOy %f\n", scene.sources->intensities.y);
	// printf("AAOz %f\n", scene.sources->intensities.z);
	cl->scene_buf = clCreateBuffer(cl->context, CL_MEM_READ_ONLY,
			sizeof(t_scene), &(scene), NULL);
