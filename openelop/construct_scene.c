...
t_vector		make_vector(double set_x, double set_y, double set_z)
{
	t_vector res;
...
////---------------------------------------------------------Plates and spheres -------------------------------------------------------------------
	...
	while (++i < 8)
	{
		...
		scene.objects[i].refractive = 0.5;
	}
	...
	while (++i < 12)
	{
		...
		// scene.objects[i].refractive = 0.5f;
	}
	...
	// scene.sources[0].position = (cl_float3){{0., 0., 1.0}};
	// scene.sources[0].type = 'd';
	// scene.sources[0].intensities = (cl_float3){{0.5, 0.5, 0.5}};
	// scene.sources[0].intensities.x = 0.5;
	// scene.sources[0].intensities.y = 0.5;
	// scene.sources[0].intensities.z = 0.5;


	// scene.sources[1].type = 'a';
	// scene.sources[1].intensities.x = 0.3;
	// scene.sources[1].intensities.y = 0.3;
	// scene.sources[1].intensities.z = 0.3;
	...
	scene.sources[0].position = (cl_float3){{5., 0., -16.5}};//scene.viewpoint;//(cl_float3){{5 * cos(phi), 5 * sin(phi), -25.0f}};
	
	//scene.sources[0].intensity = 1.f;
	//scene.sources[0].intensities = make_vector(0.1, 0.1, 1.0);
	scene.sources[0].intensities = (cl_float3){{0.1, 0.1, 1.0}};
	scene.sources[0].intensities.x = 0.1;
	scene.sources[0].intensities.y = 0.1;
	scene.sources[0].intensities.z = 1.0;
	...
	scene.sources[1].position = (cl_float3){{-5., 0., -16.5}};

	//scene.sources[1].intensity = 1.f;
	//scene.sources[1].intensities = make_vector(1.0, 0.1, 0.1);
	scene.sources[1].intensities = (cl_float3){{1.0, 0.1, 0.1}};
	scene.sources[1].intensities.x = 1.0;
	scene.sources[1].intensities.y = 0.1;
	scene.sources[1].intensities.z = 0.1;
