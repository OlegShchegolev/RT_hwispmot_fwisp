...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/08/20 12:10:10 by lvania            #+#    #+#             */
/*   Updated: 2020/08/20 12:10:13 by lvania           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			compile_cl(t_cl *cl, t_sdl *sdl)
{
	...
	while (++i < 7)
	{
		...
		(const char **)&cl->source_str, (const size_t *)&cl->source_size, NULL);
		free(cl->source_str);
		clCompileProgram(cl->programs[i], 1, &cl->dev_id, NULL, 1,
			cl->header, (const char**)header, NULL, NULL);
		compile_cl_inner(cl, i);
		progress_bar(sdl, 0.1 * i + 0.3);
	}
	cl->program = clLinkProgram(cl->context, 1, &cl->dev_id, NULL, 7,
		cl->programs, NULL, NULL, NULL);
}
...
t_cl			initcl(t_sdl *sdl)
{
	...
	compile_cl(&cl, sdl);
	clGetProgramBuildInfo(cl.program, cl.dev_id, CL_PROGRAM_BUILD_LOG, 0,
					NULL, &log_size);
	log = (char *)malloc(log_size);
	clGetProgramBuildInfo(cl.program, cl.dev_id, CL_PROGRAM_BUILD_LOG,
					log_size, log, NULL);
	free(log);
